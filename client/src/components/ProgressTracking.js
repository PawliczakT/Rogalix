...
const ProgressTracking = () => {
    ...
    return (
        <Box>
            ...
            <LinearProgress variant="determinate" value={totalRogals ? (userVotes / totalRogals) * 100 : 0}
                            sx={{height: 12, borderRadius: 6, background: '#e3f2fd'}}/>
            <Typography variant="body2" color="text.secondary" mt={1}>
                {userVotes} z {totalRogals} rogali ocenionych
                ({totalRogals ? Math.round((userVotes / totalRogals) * 100) : 0}%)
            </Typography>
        </Box>
    );
};
