    ...
    <Container>
      <Box sx={{ mt: 4 }}>
        ...
        {recommendations.length === 0 ? (
          <Typography>Nie znaleziono nowych rogali do polecenia. Oceniaj więcej rogali, by uzyskać lepsze rekomendacje!</Typography>
        ) : (
          recommendations.map(rogal => (
            <Card key={rogal._id} sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Typography variant="body2">Średnia ocena: {rogal.averageRating ? rogal.averageRating.toFixed(2) : 'Brak ocen'}</Typography>
                <Button sx={{ mt: 1 }} variant="outlined" onClick={() => navigate(`/rogals/${rogal._id}`)}>Zobacz szczegóły</Button>
              </CardContent>
            </Card>
          ))
        )}
